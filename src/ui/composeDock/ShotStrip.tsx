...
/** Shot deck for the active project. Clicking a card loads that take's rig snapshot. */
export function ShotStrip() {
  ...
  return (
    <div data-shot-strip className="flex shrink-0 flex-col border-b border-line/60">
      ...
      <div className="flex h-[4.5rem] items-stretch gap-2 overflow-x-auto px-3 pb-2">
        {ordered.length === 0 ? (
...
function ShotThumb({ shot, index }: { shot: Shot; index: number }) {
  ...
  return (
    <div
      className={`group relative flex h-full w-28 shrink-0 flex-col overflow-hidden rounded-lg text-left ${
