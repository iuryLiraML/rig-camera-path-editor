...
beforeEach(() => {
  for (const key of ['SITE_USER', 'SITE_PASSWORD', 'GOOGLE_CLIENT_ID', 'SESSION_SECRET']) {
    vi.stubEnv(key, '')
  }
})
...
describe('middleware — Google session only', () => {
  ...
  it('redirects a real navigation to /api/auth/login with returnTo, when logged out', async () => {
    vi.stubEnv('GOOGLE_CLIENT_ID', 'client-id')
    vi.stubEnv('SESSION_SECRET', 'secret')
    const res = await middleware(new Request('http://localhost/?tab=camera', navigationInit))
    expect(res?.status).toBe(302)
    const location = new URL(res!.headers.get('location')!)
    expect(location.pathname).toBe('/api/auth/login')
    expect(location.searchParams.get('returnTo')).toBe('/?tab=camera')
  })
