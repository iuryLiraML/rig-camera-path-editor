...
function safeReturnTo(value: string | undefined | null): string {
  if (!value) return '/'
  try {
    const url = new URL(value, 'https://rig.invalid')
    return url.origin === 'https://rig.invalid' && url.pathname === '/' ? `/${url.search}${url.hash}` : '/'
  } catch {
    return '/'
  }
}
...
async function handleLogin(request: Request, env: AuthEnv): Promise<Response> {
  ...
  if (!clientId || !env.GOOGLE_CLIENT_SECRET || !env.SESSION_SECRET) return textResponse(503, 'Google sign-in is not configured.')
...
<style>
  ...
  a {
    ...
    background: #454545; color: #fff; text-decoration: none; font-size: .875rem;
  }
...
async function handleMe(request: Request, env: AuthEnv): Promise<Response> {
  ...
  const email = sessionSecret
    ...
    : null
  return new Response(JSON.stringify({ email, loginConfigured: Boolean(env.GOOGLE_CLIENT_ID && env.GOOGLE_CLIENT_SECRET && env.SESSION_SECRET) }), {
